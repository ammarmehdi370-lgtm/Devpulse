...
import {
  ...
  Palette, Send, Settings2, Sparkles, Terminal, Trash2, X, Zap, AlertTriangle, History,
...
type ExecutionOutput = { stdout: string; stderr: string; exitCode: number | null; durationMs: number; timedOut: boolean; error?: string; executionId?: string; runAt?: string };
type ExecutionSummary = { id: string; language: string; exitCode: number | null; durationMs: number | null; timedOut: boolean; createdAt: string; stdoutPreview?: string; hasError: boolean };
...
const moveTreeFocus = (event: React.KeyboardEvent<HTMLElement>, direction: 1 | -1) => {
...
const FileRow: React.FC<{ file: EditorFile; active: boolean; onOpen: () => void; onMenu: (event: React.MouseEvent<HTMLButtonElement>) => void }> = ({ file, active, onOpen, onMenu }) => (
  <button role="treeitem" aria-selected={active} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); onOpen(); } if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); moveTreeFocus(event, event.key === "ArrowDown" ? 1 : -1); } }} onClick={onOpen} onContextMenu={onMenu} className={`group flex w-full items-center gap-2 border-l-2 px-3 py-1 text-left ${active ? "border-l-[#7C3AED] bg-[#1a1a2e] text-white" : "border-l-transparent text-slate-400 hover:bg-[#1a1a2e]"}`}>
...
const ApplyPreview: React.FC<{ message: AiMessage; activeFile?: EditorFile; content: string; cursor: CursorPosition; onApply: (content: string) => void }> = ({ message, activeFile, content, cursor, onApply }) => {
  ...
  const radioRefs = useRef<Record<string, HTMLLabelElement | null>>({});
  const targetKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    const enabled = targetOptions.filter((option) => option.value !== "selection" || hasSelection);
    const focused = enabled.find((option) => radioRefs.current[option.value] === document.activeElement || document.activeElement?.closest(`#target-${option.value}`));
    if (event.key === " ") { event.preventDefault(); if (focused) setTarget(focused.value); return; }
    if (!event.key.startsWith("Arrow")) return;
    event.preventDefault();
    const index = Math.max(0, enabled.findIndex((option) => option.value === (focused?.value || target)));
    const next = event.key === "ArrowDown" || event.key === "ArrowRight" ? (index + 1) % enabled.length : (index - 1 + enabled.length) % enabled.length;
    const nextTarget = enabled[next];
    if (nextTarget) { setTarget(nextTarget.value); window.requestAnimationFrame(() => radioRefs.current[nextTarget.value]?.focus()); }
  };
  return <div ref={dialogRef} role={previewOpen ? "dialog" : undefined} aria-modal={previewOpen ? "true" : undefined} aria-labelledby={previewOpen ? "diff-preview-title" : undefined} aria-describedby={previewOpen ? "diff-preview-description" : undefined} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); setPreviewOpen(false); } }} className="mt-2 border border-[#28243c] bg-[#0d0d16] p-2">
    {(!hasSelection || previewOpen) && <div className="mb-2 text-[10px] text-slate-400"><span id="apply-target-label">Apply code to:</span><div role="radiogroup" aria-labelledby="apply-target-label" onKeyDown={targetKeyDown} className="mt-1 space-y-1">{targetOptions.map((option) => <label key={option.value} ref={(element) => { radioRefs.current[option.value] = element; }} id={`target-${option.value}`} role="radio" aria-checked={target === option.value} tabIndex={option.value === "selection" && !hasSelection ? -1 : target === option.value ? 0 : -1} className="flex items-center gap-1"><input type="radio" checked={target === option.value} disabled={option.value === "selection" && !hasSelection} onChange={() => setTarget(option.value)} />{option.label}</label>)}</div></div>}
...
const PreviousConversations: React.FC<{ conversations: AiConversationSummary[]; onLoad: (conversation: AiConversationSummary) => void }> = ({ conversations, onLoad }) => {
  ...
  return <div className="border-b border-[#28243c] px-3 py-2"><button onClick={() => setExpanded((value) => !value)} className="text-[10px] text-slate-500">{conversations.length} previous conversations with this file {expanded ? "▲" : "▼"}</button>{expanded && <div className="mt-2 space-y-1">{conversations.map((conversation) => <button key={conversation.id} onClick={() => onLoad(conversation)} className="block w-full truncate px-2 py-1 text-left text-[10px] text-slate-400 hover:bg-[#1a1a2e] hover:text-white"><span>{new Date(conversation.updatedAt).toLocaleDateString()}</span><span className="ml-2 text-slate-600">{conversation.preview || conversation.title}</span></button>)}</div>}</div>;
};
...
type RevisionAuthor = { id: string; name: string; avatarUrl: string | null } | null;
type RevisionSummary = { id: string; version: number; createdAt: string; lineCount: number | null; charCount: number | null; appliedFromAI: boolean; author: RevisionAuthor };
type RevisionDetail = RevisionSummary & { content: string };

const useRevisionHistory = (fileId?: string) => {
  ...
  const load = useCallback(async (cursor?: string, append = false) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/v1/files/${fileId}/revisions?${query}`, { credentials: "include" });
      if (!response.ok) throw new Error("Unable to load revision history");
      const body = await response.json() as { revisions: RevisionSummary[]; nextCursor: string | null; total: number };
      setRevisions((previous) => append ? [...previous, ...body.revisions] : body.revisions);
      setNextCursor(body.nextCursor); setTotal(body.total);
    } finally { setLoading(false); }
  }, [fileId]);
  useEffect(() => { setRevisions([]); setNextCursor(null); void load(); }, [load]);
  const loadMore = () => { if (nextCursor && !loading) void load(nextCursor, true); };
  const getRevision = async (revisionId: string): Promise<RevisionDetail> => {
    const response = await fetch(`${API_BASE}/v1/files/${fileId}/revisions/${revisionId}`, { credentials: "include" });
    if (!response.ok) throw new Error("Unable to load revision");
    return (await response.json() as { revision: RevisionDetail }).revision;
  };
  return { revisions, nextCursor, total, loading, loadMore, getRevision };
};

const relativeRevisionTime = (createdAt: string) => {
  const date = new Date(createdAt);
  const age = Date.now() - date.getTime();
  if (age < 24 * 60 * 60 * 1000) return date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  if (age < 48 * 60 * 60 * 1000) return "Yesterday";
  return date.toLocaleDateString([], { month: "short", day: "numeric" });
};

const RestoreDialog: React.FC<{ revision: RevisionSummary; currentVersion: number; onRestore: () => void; onCancel: () => void }> = ({ revision, currentVersion, onRestore, onCancel }) => (
  <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-6" role="dialog" aria-modal="true" aria-labelledby="restore-revision-title">
    <div className="w-full max-w-sm border border-[#44346d] bg-[#161624] p-5 shadow-2xl">
      <h2 id="restore-revision-title" className="text-sm font-bold text-white">Restore to v{revision.version}?</h2>
      <p className="mt-3 text-xs leading-5 text-slate-300">Saved by {revision.author?.name || "Unknown"} • {relativeRevisionTime(revision.createdAt)}</p>
      <p className="mt-2 text-xs leading-5 text-slate-400">This will create a new revision (v{currentVersion + 1}). Current v{currentVersion} will not be deleted.</p>
      <div className="mt-5 flex justify-end gap-2 text-xs"><button onClick={onCancel} className="px-3 py-2 text-slate-400">Cancel</button><button onClick={onRestore} className="bg-[#7C3AED] px-3 py-2 font-bold text-white">Restore</button></div>
    </div>
  </div>
);

const RevisionHistoryPanel: React.FC<{
  file: EditorFile; currentContent: string; onClose: () => void; onRestore: (revision: RevisionSummary, content: string) => Promise<void>;
}> = ({ file, currentContent, onClose, onRestore }) => {
  const { revisions, nextCursor, total, loading, loadMore, getRevision } = useRevisionHistory(file.apiFileId || file.id);
  ...
  const showPreview = async (revision: RevisionSummary) => { try { setError(null); setPreview(await getRevision(revision.id)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to load revision"); } };
  if (preview) return <aside className="flex w-[320px] shrink-0 flex-col border-l border-[#28243c] bg-[#11111a]"><div className="flex h-10 shrink-0 items-center gap-2 border-b border-[#28243c] px-3"><button aria-label="Back to revision history" onClick={() => setPreview(null)} className="text-slate-400"><ChevronRight className="h-4 w-4 rotate-180" /></button><span className="font-bold text-white">v{preview.version} preview</span></div><div className="border-b border-[#28243c] px-3 py-2 text-[10px] text-slate-400">{preview.author?.name || "Unknown"} • {relativeRevisionTime(preview.createdAt)}</div><div className="min-h-0 flex-1"><DiffEditor height="100%" language={monacoLanguageForFile(file)} theme="devpulse-dark" original={preview.content} modified={currentContent} options={{ readOnly: true, originalEditable: false, minimap: { enabled: false } }} /></div><div className="flex gap-2 border-t border-[#28243c] p-3"><button onClick={() => setRestoreTarget(preview)} className="flex-1 bg-[#7C3AED] px-2 py-2 text-[10px] font-bold text-white">Restore this version</button><button onClick={() => setPreview(null)} className="px-2 py-2 text-[10px] text-slate-400">Back</button></div>{restoreTarget && <RestoreDialog revision={restoreTarget} currentVersion={currentVersion} onCancel={() => setRestoreTarget(null)} onRestore={() => void onRestore(preview, preview.content)} />}</aside>;
  return <aside className="flex w-[320px] shrink-0 flex-col border-l border-[#28243c] bg-[#11111a]"><div className="flex h-10 shrink-0 items-center gap-2 border-b border-[#28243c] px-3"><button aria-label="Close revision history" onClick={onClose} className="text-slate-400"><ChevronRight className="h-4 w-4 rotate-180" /></button><span className="font-bold text-white">Revision History</span><span className="ml-auto text-[10px] text-slate-500">{file.name}</span></div><div className="flex items-center justify-between border-b border-[#28243c] px-3 py-2 text-[10px] text-slate-500"><span>{total} revisions</span><span>All • By me</span></div><div className="min-h-0 flex-1 overflow-y-auto p-3">{error && <div className="mb-2 text-[10px] text-red-300">{error}</div>}{loading && revisions.length === 0 ? <ShimmerLines count={6} /> : revisions.length === 0 ? <div className="py-8 text-center text-[10px] text-slate-600">No revisions yet</div> : <div className="space-y-1">{revisions.map((revision, index) => <div key={revision.id} className="group border-b border-[#28243c] py-3"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-cyan-300" /><strong className="text-xs text-white">v{revision.version}</strong><span className="text-[10px] text-slate-500">{relativeRevisionTime(revision.createdAt)}</span>{index === 0 && <span className="ml-auto bg-green-400/15 px-1.5 py-0.5 text-[9px] text-green-300">Current</span>}</div><div className="mt-1 pl-4 text-[10px] text-slate-400"><span>{revision.author?.name || "Unknown"}</span><span className="ml-2 text-slate-600">{revision.lineCount ?? 0} lines</span>{revision.appliedFromAI && <span className="ml-2 text-purple-300">AI applied</span>}</div>{index !== 0 && <div className="mt-2 hidden gap-2 pl-4 group-hover:flex"><button onClick={() => void showPreview(revision)} className="text-[10px] text-cyan-300">Preview</button><button onClick={() => setRestoreTarget(revision)} className="text-[10px] text-amber-300">Restore</button></div>}{restoreTarget?.id === revision.id && <RestoreDialog revision={revision} currentVersion={currentVersion} onCancel={() => setRestoreTarget(null)} onRestore={async () => { try { const detail = await getRevision(revision.id); await onRestore(revision, detail.content); setRestoreTarget(null); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to restore revision"); } }} />}</div>)}</div>}{nextCursor && <button disabled={loading} onClick={loadMore} className="mt-3 w-full border border-[#332b50] px-3 py-2 text-[10px] text-slate-300 disabled:opacity-50">{loading ? "Loading..." : "Load more revisions"}</button>}</div></aside>;
};

const ExecutionHistory: React.FC<{ history: ExecutionSummary[]; loading: boolean; fileName: string; onSelect: (execution: ExecutionSummary) => void }> = ({ history, loading, fileName, onSelect }) => <div className="space-y-1"><div className="mb-2 text-[10px] text-slate-500">Execution History <span className="text-slate-700">{fileName}</span></div>{loading ? Array.from({ length: 3 }, (_, index) => <div key={index} className="h-7 animate-pulse bg-[#191923]" />) : history.length === 0 ? <div className="py-3 text-center text-[10px] text-slate-600">No executions yet<span className="block mt-1">Run your code with Ctrl+Enter</span></div> : history.map((execution) => <button key={execution.id} onClick={() => onSelect(execution)} className="flex w-full items-center gap-2 px-2 py-1.5 text-left text-[10px] hover:bg-[#1a1a2e]"><span className={execution.timedOut ? "text-amber-300" : execution.hasError ? "text-red-300" : "text-green-300"}>{execution.timedOut ? "⏱" : execution.hasError ? "✗" : "✓"}</span><span className="text-cyan-300">{execution.language}</span><span className="text-slate-400">{((execution.durationMs || 0) / 1000).toFixed(1)}s</span><span className="text-slate-500">{new Date(execution.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span><span className="ml-auto text-slate-600">{execution.timedOut ? "timeout" : `exit ${execution.exitCode ?? "-"}`}</span></button>)}</div>;
...
const OutputPanel: React.FC<{
  ...
  history: ExecutionSummary[]; historyLoading: boolean; fileName: string; onSelectExecution: (execution: ExecutionSummary) => void;
}> = ({ outputTab, setOutputTab, runOutput, runExitCode, isRunning, elapsedMs = 0, onRun, onClear, history, historyLoading, fileName, onSelectExecution }) => (
  <section className="flex h-[176px] shrink-0 flex-col border-t border-[#28243c] bg-[#0d0d16]">
    <div className="flex h-9 shrink-0 items-center border-b border-[#28243c] px-3">
      <div className="flex h-full items-center gap-4">{(["terminal", "output", "problems", "history"] as OutputTab[]).map((tab) => <button key={tab} onClick={() => setOutputTab(tab)} className={`h-full text-[10px] font-bold tracking-wider ${outputTab === tab ? "border-b-2 border-purple-400 text-white" : "text-slate-500"}`}>{tab.toUpperCase()}</button>)}</div>
    ...
    <div className="min-h-0 flex-1 overflow-auto p-3 text-[11px] leading-5">{outputTab === "history" ? <ExecutionHistory history={history} loading={historyLoading} fileName={fileName} onSelect={onSelectExecution} /> : outputTab === "problems" ? <div className="flex items-center gap-2 text-slate-600"><CircleDot className="h-3.5 w-3.5" />No problems detected</div> : outputTab === "output" ? <div className="text-slate-500">Build and diagnostic output will appear here.</div> : <div className={runExitCode !== null && runExitCode !== 0 ? "text-red-300" : "text-green-300"}><div className="mb-1 flex items-center gap-2 text-[10px]">{runExitCode === null ? <CircleDot className="h-3 w-3 text-slate-500" /> : runExitCode === 0 ? <Check className="h-3 w-3" /> : <CircleX className="h-3 w-3" />}<span>{isRunning ? `Running... ${ (elapsedMs / 1000).toFixed(1) }s` : runExitCode === null ? "Ready" : `Exited with code ${runExitCode} ${runExitCode === 0 ? "✓" : "✗"}`}</span>{!isRunning && runExitCode !== null && <span>Completed in {(elapsedMs / 1000).toFixed(1)}s</span>}{isRunning && elapsedMs > 10000 && <span>Still running... (timeout in {Math.max(0, 30 - Math.floor(elapsedMs / 1000))}s)</span>}</div><pre className="whitespace-pre-wrap"><span className="text-purple-400">$</span> <span className="text-cyan-300">devpulse terminal</span>{"\n"}{runOutput}{isRunning && <span className="animate-pulse text-cyan-300"> ▌</span>}</pre></div>}</div>
  </section>
);

type ConflictDetails = { message?: string; lastEditedBy?: string | null; lastEditedAt?: string | null; currentVersion?: number; yourVersion?: number };

const ConflictModal: React.FC<{ file: EditorFile; projectId: string; mine: string; details: ConflictDetails; onKeepMine: () => Promise<void>; onKeepTheirs: () => Promise<void>; onClose: () => void }> = ({ file, projectId, mine, details, onKeepMine, onKeepTheirs, onClose }) => {
  const [serverContent, setServerContent] = useState<string | null>(null);
  const [serverVersion, setServerVersion] = useState<number | null>(details.currentVersion ?? null);
  const [compare, setCompare] = useState(false);
  useEffect(() => {
    let cancelled = false;
    void fetch(`${API_BASE}/v1/projects/${projectId}/files/${file.apiFileId || file.id}`, { credentials: "include" }).then(async (response) => response.ok ? response.json() as Promise<{ content: string; version: number }> : null).then((latest) => { if (!cancelled && latest) { setServerContent(latest.content); setServerVersion(latest.version); } }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [file.apiFileId, file.id, projectId]);
  const formattedEditedAt = details.lastEditedAt ? new Date(details.lastEditedAt).toLocaleString() : "just now";
  if (compare && serverContent !== null) return <div className="fixed inset-0 z-50 flex flex-col bg-[#0A0A0F]" role="dialog" aria-modal="true" aria-labelledby="compare-title"><div className="flex items-center justify-between border-b border-[#28243c] px-4 py-3"><h2 id="compare-title" className="text-sm font-bold text-white">Compare save conflict: {file.name}</h2><button onClick={() => setCompare(false)} className="text-xs text-slate-400">Back</button></div><div className="flex min-h-0 flex-1"><div className="flex min-w-0 flex-1 flex-col border-r border-[#28243c]"><div className="px-3 py-2 text-xs text-amber-200">Your version</div><div className="min-h-0 flex-1"><MonacoEditor height="100%" language={monacoLanguageForFile(file)} theme="devpulse-dark" value={mine} options={{ readOnly: true }} /></div><button onClick={() => void onKeepMine()} className="m-3 bg-amber-500/20 px-3 py-2 text-xs text-amber-100">Use this version</button></div><div className="flex min-w-0 flex-1 flex-col"><div className="px-3 py-2 text-xs text-cyan-200">Server version</div><div className="min-h-0 flex-1"><MonacoEditor height="100%" language={monacoLanguageForFile(file)} theme="devpulse-dark" value={serverContent} options={{ readOnly: true }} /></div><button onClick={() => void onKeepTheirs()} className="m-3 bg-cyan-400/15 px-3 py-2 text-xs text-cyan-200">Use this version</button></div></div></div>;
...
export const EditorWorkbench: React.FC = () => {
  ...
  const executionOutputs = useRef<Record<string, ExecutionOutput>>({});
  ...
  const [previousConversations, setPreviousConversations] = useState<AiConversationSummary[]>([]);
  ...
  const [conflict, setConflict] = useState<{ fileId: string; mine: string; details: ConflictDetails } | null>(null);
  const [conflictedFileIds, setConflictedFileIds] = useState<string[]>([]);
  const [presenceByFile, setPresenceByFile] = useState<Record<string, { userId: string; name: string }>>({});
  ...
  const loadExecutionHistory = useCallback(async (fileId: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/v1/files/${fileId}/executions?limit=10`, { credentials: "include" });
      if (response.ok) setExecutionHistory((await response.json() as { executions: ExecutionSummary[] }).executions);
    } catch { setExecutionHistory([]); } finally { setExecutionHistoryLoading(false); }
  }, []);
  const handleOutputTabChange = (tab: OutputTab) => { setOutputTab(tab); if (tab === "history" && activeFile?.origin === FileOrigin.API) void loadExecutionHistory(activeFile.apiFileId || activeFile.id); };
  const selectHistoricalExecution = async (summary: ExecutionSummary) => {
    const response = await fetch(`${API_BASE}/v1/executions/${summary.id}`, { credentials: "include" });
    if (!response.ok) return;
    const body = await response.json() as { execution: ExecutionOutput & { codeSnapshot: string; createdAt: string } };
    const execution = { ...body.execution, runAt: body.execution.createdAt };
    if (activeFile) executionOutputs.current[activeFile.id] = execution;
    setRunOutput(`Run from ${new Date(execution.createdAt).toLocaleTimeString()}\n\n${execution.stdout || execution.stderr || execution.error || "Process completed with no output."}`);
    setRunExitCode(execution.exitCode); setElapsedMs(execution.durationMs); setOutputTab("terminal");
  };
  useEffect(() => {
    if (!activeFile) return;
    const saved = executionOutputs.current[activeFile.id];
    setRunOutput(saved ? `Run from ${saved.runAt ? new Date(saved.runAt).toLocaleTimeString() : "previous run"}\n\n${saved.stdout || saved.stderr || saved.error || "Process completed with no output."}` : "Run your code to see output.");
    setRunExitCode(saved?.exitCode ?? null); setElapsedMs(saved?.durationMs ?? 0);
  }, [activeFile?.id]);
  useEffect(() => { if (isOutputOpen && activeFile?.origin === FileOrigin.API) void loadExecutionHistory(activeFile.apiFileId || activeFile.id); }, [isOutputOpen, activeFile?.id, activeFile?.apiFileId, loadExecutionHistory]);
  ...
  useEffect(() => {
    ...
    client.emit("workspace:join", editorProjectId);
    client.on("file:editing", (payload: { fileId: string; userId: string; userName?: string }) => { if (payload.userId !== user.email) setPresenceByFile((previous) => ({ ...previous, [payload.fileId]: { userId: payload.userId, name: payload.userName || "A collaborator" } })); });
    return () => { client.emit("workspace:leave", editorProjectId); client.disconnect(); socketRef.current = null; setPresenceByFile({}); };
  }, [editorProjectId, user.email]);
  useEffect(() => { if (!activeFile || !editorProjectId) return; socketRef.current?.emit("file:editing", { workspaceId: editorProjectId, fileId: activeFile.apiFileId || activeFile.id, userId: user.email, userName: user.name || user.email }); }, [activeFile?.id, activeFile?.apiFileId, editorProjectId, user.email, user.name]);
  ...
  const cacheAiMessages = (fileId: string, nextMessages: AiMessage[]) => { localStorage.setItem(`dp-ai-history-${fileId}`, JSON.stringify(nextMessages.slice(-20))); };
  const dbMessageToUi = (message: AiDbMessage): AiMessage => ({ id: message.id, role: message.role === "USER" ? "user" : "assistant", text: message.content, command: AI_COMMANDS.some((command) => command.value === message.command) ? message.command as AiCommand : undefined, fileId: activeFile?.id });
  const mergeAiHistory = (localMessages: AiMessage[], dbMessages: AiDbMessage[]) => {
    const merged = [...dbMessages.slice().reverse().map(dbMessageToUi), ...localMessages];
    const seen = new Set<string>();
    return merged.filter((message) => { const key = `${message.role}:${message.text}`; if (seen.has(key)) return false; seen.add(key); return true; }).slice(-20);
  };
  const persistAiMessage = async (conversationId: string, message: AiMessage, tokensUsed?: number) => {
    await fetch(`${API_BASE}/v1/ai/conversations/${conversationId}/messages`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ role: message.role, content: message.text, command: message.command, tokensUsed, hasCode: message.role === "assistant" && message.text.includes("```"), codeLanguage: message.role === "assistant" ? extractCodeLanguage(message.text) : undefined }) }).catch(() => undefined);
  };
  const syncAiHistory = async (fileId: string, localMessages: AiMessage[]) => {
    try {
      const conversationResponse = await fetch(`${API_BASE}/v1/ai/conversations?fileId=${encodeURIComponent(fileId)}`, { credentials: "include" });
      if (!conversationResponse.ok) return;
      const conversationBody = await conversationResponse.json() as { conversation: AiConversationSummary };
      setConversation(conversationBody.conversation);
      const previousResponse = await fetch(`${API_BASE}/v1/ai/conversations?fileId=${encodeURIComponent(fileId)}&all=true`, { credentials: "include" });
      if (previousResponse.ok) setPreviousConversations(((await previousResponse.json()) as { conversations: AiConversationSummary[] }).conversations.filter((item) => item.id !== conversationBody.conversation.id));
      const messagesResponse = await fetch(`${API_BASE}/v1/ai/conversations/${conversationBody.conversation.id}/messages?limit=20`, { credentials: "include" });
      const dbMessages = messagesResponse.ok ? ((await messagesResponse.json()) as { messages: AiDbMessage[] }).messages : [];
      const merged = mergeAiHistory(localMessages, dbMessages);
      setMessages(merged); setHistoryCount(merged.length); cacheAiMessages(fileId, merged);
      const dbKeys = new Set(dbMessages.map((message) => `${message.role === "USER" ? "user" : "assistant"}:${message.content}`));
      await Promise.all(localMessages.filter((message) => !message.streaming && !dbKeys.has(`${message.role}:${message.text}`)).map((message) => persistAiMessage(conversationBody.conversation.id, message)));
    } catch { /* local cache remains the usable fallback */ }
  };
  ...
  useEffect(() => {
    ...
    try { const stored = JSON.parse(localStorage.getItem(`dp-ai-history-${activeFileId}`) || "[]") as AiMessage[]; setMessages(stored); setHistoryCount(stored.length); if (activeFile?.origin === FileOrigin.API) void syncAiHistory(activeFile.apiFileId || activeFile.id, stored); } catch { setMessages([]); setHistoryCount(0); }
  }, [activeFileId]);
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.altKey && event.key.toLowerCase() === "h" && activeFile?.origin === FileOrigin.API) { event.preventDefault(); setIsContributorPanelOpen(false); setIsAiDrawerOpen(false); setIsRevisionPanelOpen((open) => !open); return; }
      ...
      else if (event.key.toLowerCase() === "i") { event.preventDefault(); setIsContributorPanelOpen(false); setIsRevisionPanelOpen(false); setIsAiDrawerOpen((open) => !open); }
  ...
  const saveActiveFile = async () => {
    ...
    catch (error) { setSaveState("error"); if (error instanceof ApiRequestError && error.status === 409) { setConflictedFileIds((previous) => previous.includes(activeFile.id) ? previous : [...previous, activeFile.id]); setConflict({ fileId: activeFile.id, mine: activeContent, details: error.details as ConflictDetails }); } setRunOutput(error instanceof Error ? error.message : "Save failed"); }
  };
  ...
  useEffect(() => {
    if (!activeFile || activeFile.origin !== FileOrigin.API || !activeFile.modified || conflictedFileIds.includes(activeFile.id)) return;
    const timer = window.setTimeout(() => void saveActiveFile(), 2000);
    return () => window.clearTimeout(timer);
  }, [activeFile?.id, activeFile?.modified, activeContent, conflictedFileIds]);
  useEffect(() => {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const durationMs = Date.now() - startedAt; const exitCode = result.exitCode ?? (response.ok ? 0 : 1); const output: ExecutionOutput = { stdout: result.stdout || "", stderr: result.stderr || "", exitCode, durationMs, timedOut: Boolean((result as { timedOut?: boolean }).timedOut), error: result.error, executionId: result.executionId, runAt: new Date().toISOString() }; executionOutputs.current[activeFile.id] = output; setElapsedMs(durationMs); setRunExitCode(exitCode); setRunOutput(`$ devpulse run ${activeFile.name}\n\n${output.stdout || output.stderr || output.error || "Process completed with no output."}`); if (activeFile.origin === FileOrigin.API) void loadExecutionHistory(activeFile.apiFileId || activeFile.id);
    } catch (error) { const durationMs = Date.now() - startedAt; const output: ExecutionOutput = { stdout: "", stderr: "", exitCode: 1, durationMs, timedOut: false, error: error instanceof Error ? error.message : "Execution service unavailable", runAt: new Date().toISOString() }; executionOutputs.current[activeFile.id] = output; setElapsedMs(durationMs); setRunExitCode(1); setRunOutput(output.error || "Execution service unavailable"); }
  ...
  const clearAiHistory = async () => { if (!activeFileId) return; localStorage.removeItem(`dp-ai-history-${activeFileId}`); setMessages([]); setHistoryCount(0); const oldConversation = conversation; if (oldConversation) await fetch(`${API_BASE}/v1/ai/conversations/${oldConversation.id}`, { method: "DELETE", credentials: "include" }).catch(() => undefined); if (activeFile?.origin === FileOrigin.API) { const response = await fetch(`${API_BASE}/v1/ai/conversations?fileId=${encodeURIComponent(activeFile.apiFileId || activeFile.id)}`, { credentials: "include" }).catch(() => null); if (response?.ok) setConversation(((await response.json()) as { conversation: AiConversationSummary }).conversation); } };
  ...
  const streamAiResponse = async (prompt: string, command?: AiCommand, messageSelection?: SelectionSnapshot | null) => {
    aiAbortRef.current?.abort(); const controller = new AbortController(); aiAbortRef.current = controller; const assistantId = `assistant-${Date.now()}`; let assistantContent = "";
    ...
    finally { setMessages((previous) => { const next = previous.map((message) => message.id === assistantId ? { ...message, text: assistantContent || message.text, streaming: false } : message); if (activeFileId) { cacheAiMessages(activeFileId, next); setHistoryCount(next.length); } return next; }); if (assistantContent && conversation?.id) void persistAiMessage(conversation.id, { id: assistantId, role: "assistant", text: assistantContent, fileId: activeFileId }); setIsStreaming(false); }
  };
  const sendAi = (event: React.FormEvent) => { event.preventDefault(); const rawPrompt = aiInput.trim(); if (!rawPrompt || isStreaming || isLargeFileBlocked || usageExhausted) return; const commandMatch = AI_COMMANDS.find((command) => rawPrompt.startsWith(command.value)); const prompt = commandMatch ? rawPrompt.slice(commandMatch.value.length).trim() : rawPrompt; const messageSelection = selection; if (commandMatch?.requiresSelection && !messageSelection?.selectedText) setRunOutput("Tip: Select some code first for more precise results"); const message: AiMessage = { id: `user-${Date.now()}`, role: "user", text: prompt, command: commandMatch?.value, fileId: activeFile?.id, selection: messageSelection || undefined }; setMessages((previous) => { const next = [...previous, message]; if (activeFileId) cacheAiMessages(activeFileId, next); setHistoryCount(next.length); return next; }); if (conversation?.id) void persistAiMessage(conversation.id, message); setAiInput(""); void streamAiResponse(prompt, commandMatch?.value, messageSelection); };
  const loadConversation = async (selected: AiConversationSummary) => { const response = await fetch(`${API_BASE}/v1/ai/conversations/${selected.id}/messages?limit=20`, { credentials: "include" }); if (!response.ok) return; const body = await response.json() as { messages: AiDbMessage[] }; const loaded = body.messages.slice().reverse().map(dbMessageToUi); setConversation(selected); setMessages(loaded); setHistoryCount(loaded.length); if (activeFileId) cacheAiMessages(activeFileId, loaded); };
  const exportConversation = async () => { if (!conversation) return; const response = await fetch(`${API_BASE}/v1/ai/conversations/${conversation.id}/export`, { credentials: "include" }); if (!response.ok) return; const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `devpulse-ai-${new Date().toISOString().slice(0, 10)}.md`; link.click(); URL.revokeObjectURL(url); };
  ...
  const handleContextAction = async (action: "new" | "rename" | "delete" | "copy", file: EditorFile) => {
    ...
    else { const nextPath = window.prompt("Rename file", file.path); if (nextPath && nextPath !== file.path) { await fetch(`${API_BASE}/v1/files/${file.id}`, { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: nextPath, expectedVersion: file.apiVersion ?? file.version }) }); await loadEditorProject(); } }
  };
  ...
  const conflictFile = conflict ? openFiles.find((file) => file.id === conflict.fileId) : undefined;
  const keepMine = async () => { if (!conflictFile || !conflict) return; setSaveState("saving"); try { await saveFileContent(conflictFile.id, conflict.mine, true); setConflictedFileIds((previous) => previous.filter((id) => id !== conflictFile.id)); setConflict(null); setSaveState("saved"); } catch (error) { setSaveState("error"); setRunOutput(error instanceof Error ? error.message : "Force save failed"); } };
  const keepTheirs = async () => { setConflict(null); setConflictedFileIds((previous) => conflict ? previous.filter((id) => id !== conflict.fileId) : previous); await loadEditorProject(); setSaveState("idle"); };
  const restoreRevision = async (revision: RevisionSummary, content: string) => {
    if (!activeFile || activeFile.origin !== FileOrigin.API) return;
    setSaveState("saving");
    try {
      const response = await fetch(`${API_BASE}/v1/files/${activeFile.apiFileId || activeFile.id}/restore`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ revisionId: revision.id, expectedVersion: activeFile.version }) });
      const body = await response.json() as { file?: { version: number }; error?: string };
      if (!response.ok) throw new Error(body.error || "Unable to restore revision");
      updateFileContent(activeFile.id, content);
      await loadEditorProject();
      setIsRevisionPanelOpen(false); setSaveState("saved"); setAiApplyNotice(`Restored to v${revision.version} ✓ (now saved as v${body.file?.version ?? activeFile.version + 1})`); window.setTimeout(() => setAiApplyNotice(null), 4000); window.setTimeout(() => setSaveState("idle"), 2000);
    } catch (reason) { setSaveState("error"); setAiApplyNotice(reason instanceof Error ? reason.message : "Unable to restore revision"); }
  };
  ...
  return <div aria-busy="false" className="flex h-[calc(100vh-3.5rem)] min-h-0 flex-col overflow-hidden bg-[#0A0A0F] font-mono text-xs text-slate-300">
    ...
    {conflict && conflictFile && <ConflictModal file={conflictFile} projectId={editorProjectId} mine={conflict.mine} details={conflict.details} onKeepMine={keepMine} onKeepTheirs={keepTheirs} onClose={() => setConflict(null)} />}
