...
export const WorkspacesPage: React.FC = () => {
  ...
  const templates = [
    { id: 'Next.js 15', name: 'Next.js 15', sub: 'Turbopack - SSR', icon: '⚡' },
    { id: 'FastAPI Python', name: 'FastAPI Python', sub: 'Python 3.12 - Uvicorn', icon: '🐍' },
    { id: 'Rust Axum', name: 'Rust Axum', sub: 'Tokio - Cargo LLVM', icon: '🦀' },
    { id: 'Go gRPC', name: 'Go gRPC', sub: 'Go 1.23 - Protobuf', icon: '🔷' },
    { id: 'PyTorch AI Agent', name: 'PyTorch AI Agent', sub: 'CUDA runtime - HuggingFace accelerated', icon: '🧠' },
  ];
  ...
  const handleCloneRepo = () => {
    ...
    setIsCloning(true);
    setTimeout(() => {
      const repoName = cloneUrl.split('/').pop()?.replace('.git', '') || 'cloned-workspace';
  ...
  const handleSpinUpTemplate = () => {
    spinUpDevbox(`${selectedTemplate.toLowerCase().replace(/[^a-z0-9]/g, '-')}-box`, selectedTemplate);
  };

  const handleCopyCli = () => {
    navigator.clipboard.writeText('curl -fsSL https://codeplane.dev/install.sh | sh');
    setCliCopied(true);
  ...
  return (
    <div className="p-6 sm:p-8 max-w-[1240px] mx-auto space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-[#84849e] max-w-2xl leading-relaxed">
            Spin up an isolated containerized environment in under 2 seconds. Fully ephemeral, synchronized, and pre-configured.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="bg-[#12121b] border border-[#202030] rounded-xl p-3 text-right font-mono text-xs">
            <div className="text-[10px] text-[#6b6b88] uppercase tracking-wider">ACTIVE FLEET REGION</div>
            <div className="text-[#0DF5C4] font-bold mt-0.5">us-east-nitro-4</div>
          </div>
          <div className="bg-[#12121b] border border-[#202030] rounded-xl p-3 text-right font-mono text-xs">
            <div className="text-[10px] text-[#6b6b88] uppercase tracking-wider">WARM WARMUP POOL</div>
            <div className="text-[#0DF5C4] font-bold mt-0.5">1.18s avg boot</div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-[#12121b] border border-[#222234] rounded-2xl p-6 flex flex-col justify-between shadow-xl relative overflow-hidden">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-[#62627e] uppercase tracking-wider">OPTION 01</span>
            </div>

            <div>
              <h3 className="text-lg font-bold text-white mb-1">Clone a Git Repository</h3>
              <p className="text-xs text-[#8c8ca5] leading-relaxed">
                Import directly from GitHub, GitLab, or any public/private Git URL with automatic environment detection.
              </p>
            </div>
          ...
          <div className="pt-6">
            ...
            >
              <span>{isCloning ? 'Cloning & Provisioning...' : 'Import Repository'}</span>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-[#0DF5C4]/10 text-[#0DF5C4] border border-[#0DF5C4]/30">Instant Boot</span>
                <span className="text-[11px] font-mono text-[#62627e] uppercase tracking-wider">OPTION 02</span>
              </div>
            </div>

            <div>
              <h3 className="text-lg font-bold text-white mb-1">Start from a Template</h3>
              <p className="text-xs text-[#8c8ca5] leading-relaxed">
                Curated, production-ready microarchitectures configured with package caching and hot-reload daemon.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              {templates.slice(0, 4).map((tpl) => (
                ...
                >
                  ...
                  <div className="text-[10px] text-[#63637e] font-mono truncate">{tpl.sub}</div>
                </button>
              ))}
            </div>
            ...
            >
              ...
              <div className="text-[10px] text-[#63637e] font-mono">CUDA runtime · HuggingFace accelerated</div>
            </button>
          </div>
        ...
        <div className="bg-[#12121b] border border-[#222234] rounded-2xl p-6 flex flex-col justify-between shadow-xl relative overflow-hidden">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-[#62627e] uppercase tracking-wider">OPTION 03</span>
            </div>

            <div>
              <h3 className="text-lg font-bold text-white mb-1">Connect Local Machine</h3>
              <p className="text-xs text-[#8c8ca5] leading-relaxed">
                Bridge your native laptop environment directly to high-spec cloud devboxes via our zero-config agent.
              </p>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="relative flex items-center">
                <input
                  type="text"
                  readOnly
                  value="curl -fsSL https://codeplane.dev/i: |"
                ...
                <button
                  ...
                  title="Copy command"
                >
                  {cliCopied ? <Check className="w-3.5 h-3.5 text-[#0DF5C4]" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
      ...
      <div className="bg-[#101017] border border-[#20202e] rounded-2xl p-6 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1c1c2a]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Recent Workspaces & Starred Repositories</h3>
          ...
          <div className="flex items-center gap-3 text-xs font-mono">
            <button 
              onClick={() => alert('Codeplane Cloud Documentation: Hypervisor MicroVM Specs & Architecture')}
        ...
        {workspaces.length === 0 ? (
          <div className="p-8 text-center text-xs text-[#777792]">
            You don't have any running workspaces yet. Once launched, your compute instances, pinned ports, and environment secrets will populate here for fast resumption.
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono flex items-center gap-1 ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${ws.status === 'Running' ? 'bg-[#0DF5C4] animate-pulse' : 'bg-[#FF9E64]'}`} />
                        {ws.status}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      title={ws.status === 'Running' ? 'Stop Workspace' : 'Start Workspace'}
                      className="p-1.5 rounded-lg bg-[#1c1c2b] hover:bg-[#252538] text-[#a0a0be] hover:text-white transition-colors"
                    >
                      {ws.status === 'Running' ? <Square className="w-3.5 h-3.5 text-[#FF9E64]" /> : <Play className="w-3.5 h-3.5 text-[#0DF5C4]" />}
                    </button>
                    <button
      ...
      <div className="bg-[#101017] border border-[#20202e] rounded-2xl p-6 shadow-xl space-y-6">
        <div>
          ...
          <p className="text-xs text-[#8888a2] max-w-3xl mt-1 leading-relaxed">
            Shared-nothing microkernels boot with dedicated NVMe write caches, zero-trust ephemeral peer rings, and pre-warmed language LSP servers.
          </p>
        ...
        <div className="h-44 bg-[#0a0a10] border border-[#1d1d2b] rounded-xl p-4 flex items-center justify-around relative overflow-hidden font-mono text-xs">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none stroke-[#2a2a44]" strokeWidth="1.5" strokeDasharray="4 4">
            <line x1="20%" y1="50%" x2="50%" y2="25%" className="stroke-[#0DF5C4]/60" />
            <line x1="20%" y1="50%" x2="50%" y2="75%" className="stroke-[#FF9E64]/60" />
            <line x1="50%" y1="25%" x2="80%" y2="50%" className="stroke-[#0DF5C4]/60" />
            <line x1="50%" y1="75%" x2="80%" y2="50%" className="stroke-[#FF9E64]/60" />
          </svg>
          ...
          <div className="z-10 text-center">
            <div 
              ...
              style={{ backgroundColor: `${theme.primary}30`, borderColor: theme.primary, borderWidth: 1 }}
            >
      ...
      <div className="pt-2 pb-4 flex flex-wrap items-center justify-between gap-4 text-xs font-mono text-[#6d6d88] border-t border-[#1a1a28]">
        <div className="flex items-center gap-2">
          <span className="text-[#8c8ca5] font-semibold">PRODUCTIVITY ACCELERATORS:</span>
          <span>⚡ Quick command palette</span>
          <span>·</span>
          <span>Jump to file & symbol</span>
        </div>
        <div className="flex items-center gap-3">
          <span><kbd className="px-1.5 py-0.5 rounded bg-[#161624] border border-[#27273a] text-white">Ctrl+`</kbd> Open integrated terminal</span>
          <span>Press <kbd className="px-1 rounded bg-[#161624] border border-[#27273a] text-white">?</kbd> for keyboard map</span>
        </div>
      </div>
