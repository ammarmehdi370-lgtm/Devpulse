...
const DEFAULT_FILE_CONTENTS: Record<string, string> = {
  "file-1": `import { createServer } from 'http';
import { processQueue } from './workers/telemetry.js';

const PORT = process.env.PORT || 3000;

export async function startRuntime() {
  console.log(\`[DEVPULSE] Initializing isolated microkernel on port \${PORT}...\`);
  ...
  server.listen(PORT, () => {
    console.log(\`[DEVPULSE] Cloud devbox online at http://localhost:\${PORT}\`);
  });
}

startRuntime();
`,
  "file-2": `// Devpulse WebSocket & Mesh Network Layer
...
export function pingMesh(nodes: MeshNode[]): Promise<MeshNode[]> {
  return Promise.resolve(nodes.map(n => ({ ...n, latencyMs: Math.floor(Math.random() * 15) + 5 })));
}
`,
  "file-3": `# Autonomous Neural Coding Agent Daemon
import sys
import time

def run_agent():
    print("[AI-AGENT] PyTorch CUDA 12.4 worker initialized.")
    print("[AI-AGENT] KV Cache memory mapped: 32GB ECC RAM.")
    for step in range(1, 4):
        time.sleep(0.1)
        print(f"[AI-AGENT] Step {step}/3: AST mutation verified. Zero syntax faults.")
    print("[AI-AGENT] Ready for developer instructions.")
...
`,
  ...
  "name": "devpulse-core",
  "version": "2.4.0",
  "description": "Hyper-optimized microVM runtime and ephemeral orchestration layer.",
  "type": "module",
  "scripts": {
...
`,
  "file-5": `# devpulse-core

Hyper-optimized microVM runtime, ephemeral orchestration layer, and LSP bridge.

## Quick Start
1. Click **Run Code** to execute this file in the cloud Devbox.
2. Ask the **AI Assistant** on the right to optimize or rewrite functions.
3. Switch files in the explorer tree on the left.
`,
};
...
const INITIAL_ENV_VARS: EnvVariable[] = [
  ...
  },
  {
    id: "env-2",
    key: "OPENAI_API_KEY",
    value: "REDACTED_IN_CLIENT_PREVIEW",
