...
function createAccessToken(user) {
    return jsonwebtoken_1.default.sign({ sub: user.id, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
}
...
async function createVerificationToken(userId, email) {
    ...
    await database_1.database.query('DELETE FROM email_verification_tokens WHERE user_id = $1', [userId]);
    await database_1.database.query(`INSERT INTO email_verification_tokens (user_id, token_hash, expires_at)
     VALUES ($1, $2, $3)`, [userId, hashToken(token), new Date(Date.now() + TOKEN_TTL_MS)]);
...
router.post('/register', async (req, res) => {
    ...
    try {
        const existing = await database_1.database.query('SELECT id, email_verified_at FROM users WHERE email = $1', [email]);
        if (existing.rowCount) {
            return res.status(409).json({ success: false, error: 'An account with this email already exists.' });
        }
        const passwordHash = await bcryptjs_1.default.hash(password, 12);
        const result = await database_1.database.query('INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id, email', [email, passwordHash]);
...
router.get('/verify-email', async (req, res) => {
    ...
    try {
        const result = await database_1.database.query(`SELECT u.id, u.email
       FROM email_verification_tokens t
       JOIN users u ON u.id = t.user_id
       WHERE t.token_hash = $1 AND t.expires_at > CURRENT_TIMESTAMP`, [hashToken(token)]);
        if (!result.rowCount)
            return res.status(400).json({ success: false, error: 'Token is invalid or expired.' });
        const user = result.rows[0];
        await database_1.database.query('UPDATE users SET email_verified_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [user.id]);
        await database_1.database.query('DELETE FROM email_verification_tokens WHERE user_id = $1', [user.id]);
...
router.post('/resend-verification', async (req, res) => {
    const email = normalizeEmail(req.body.email);
    try {
        const result = await database_1.database.query('SELECT id, email, email_verified_at FROM users WHERE email = $1', [email]);
...
router.post('/login', async (req, res) => {
    ...
    const password = typeof req.body.password === 'string' ? req.body.password : '';
    try {
        const result = await database_1.database.query('SELECT id, email, password_hash, email_verified_at FROM users WHERE email = $1', [email]);
        const user = result.rows[0];
